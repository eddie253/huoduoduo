...
其中部分需求需要 BFF (apps/bff_gateway) 提供對應 API 或後端保護機制。
...
| 範圍 | 說明 |
| --- | --- |
| ✅ | apps/bff_gateway (NestJS) 之 controller/service/DTO/Redis adapters |
...
- `apps/bff_gateway/README.md` 記錄新的 Redis key 及啟動前置需求。
...
| 里程碑 | Owner | 內容 |
| --- | --- | --- |
| M1 | BFF Team | 完成 P29-1 / P29-2，佈建 Redis guard + DTO 調整 |
| M2 | BFF Team | 完成 Orders module (P29-3) + 單元/整合測試 |
| M3 | BFF Team | 完成 Driver Location APIs & background flusher |
| M4 | Docs Team | 更新 PLAN25/27/OpenAPI/README |
| M5 | QA | 真機測試：Flutter (PLAN25–27) + BFF (PLAN29) 整合驗證 |
...
1. [ ] 建立 `apps/bff_gateway/src/modules/orders`、`driver-location` 目錄 + 測試
2. [ ] 實作 `IdempotencyGuardService` + Redis provider
3. [ ] 更新 OpenAPI、README、Plan docs
4. [ ] `npm run test`, `npm run bff:start:local` smoke test
5. [ ] 與 Flutter 團隊共同驗證（Jenkins Android build + 真機）
