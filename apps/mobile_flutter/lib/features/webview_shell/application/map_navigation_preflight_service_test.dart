...
void main() {
  ...
  test('allows when google maps app is unavailable in non-strict mode',
      () async {
    final result = await _createService(
  ...
  test('blocks when google maps app is unavailable in strict mode', () async {
    final result = await _createService(
      mapsAvailabilityPort:
          const _FakeGoogleMapsAvailabilityPort(available: false),
      blockWhenGoogleMapsUnavailable: true,
    ).ensureReady();
  ...
  test('allows when Google account is missing on Android in non-strict mode',
      () async {
    final result = await _createService(
  ...
  test(
      'allows when Google account state is unknown on Android in non-strict mode',
      () async {
    final result = await _createService(
      googleAccountPort: const _FakeGoogleAccountPort(
  ...
  test('blocks when Google account is missing on Android in strict mode',
      () async {
    final result = await _createService(
      googleAccountPort: const _FakeGoogleAccountPort(
        stateValue: GoogleAccountState.missing,
      ...
      requireGoogleAccountOnAndroid: true,
      blockWhenGoogleAccountUnverified: true,
    ).ensureReady();
  ...
  test('blocks when Google account state is unknown on Android in strict mode',
      () async {
    final result = await _createService(
      googleAccountPort: const _FakeGoogleAccountPort(
        stateValue: GoogleAccountState.unknown,
...
DefaultMapNavigationPreflightService _createService({
  ...
  bool blockWhenGoogleAccountUnverified = false,
  bool blockWhenGoogleMapsUnavailable = false,
}) {
  return DefaultMapNavigationPreflightService(
    ...
    blockWhenGoogleAccountUnverified: blockWhenGoogleAccountUnverified,
    blockWhenGoogleMapsUnavailable: blockWhenGoogleMapsUnavailable,
  );
}
